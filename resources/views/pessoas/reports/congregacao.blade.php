<html>
    ...
    <body>
        ...
        <div style="border: 0.1mm solid black;padding:0.5em; background-color: rgba(211, 211, 211, 1.0); text-align:center;">
            <strong>LISTAGEM DE IRMÃS POR CONGREGAÇÃO</strong>
        </div>
            ...
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        @if ($congregacoes->count())
                            <table cellspacing="0" style="width: 100%; text-align: left; font-size: 10pt;">
                                <thead>
                                    <tr>
                                        <th width="300" >{{ __('NOME COMPLETO') }}</th>
                                        <th width="80" style="text-align:center">{{ __('CELULAR') }}</th>
                                        <th width="100" style="text-align:center">{{ __('CONGREGAÇÃO') }}</th>
                                    </tr>
                                </thead>
                                @foreach ($congregacoes as $congregacao)
                                    <tbody>   
                                        <tr>
                                            <td class="alt" width="300" >{{$congregacao->name}}</td>
                                            <td class="alt" width="80" style="text-align:center">{{$congregacao->celular}}</td>
                                            <td class="alt" width="100" style="text-align:center">{{$congregacao->congregacao->name}}</td>
                                        </tr>  
                                    </tbody>                                  
                                @endforeach
                                <tr>
                                    <td colspan="4" style="widtd: 35%; text-align: right;"><strong>Total de Irmãs.:</strong></td>
                                    <td style="widtd: 15%; text-align: right;"><strong>{{$countCongregacoes}}</strong></td>
                                </tr>
                            </table>                                
                        @else
