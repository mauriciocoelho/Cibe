<html>
    ...
    <body>
        ...
        <div style="border: 0.1mm solid black;padding:0.5em; background-color: rgba(211, 211, 211, 1.0); text-align:center;">
            <strong>RELATÓRIO DE PAGOS</strong>
        </div>
                        ...
                        @if ($pagos->count())
                            <table cellspacing="0" style="width: 100%; text-align: left; font-size: 10pt;">
                                ...
                                @foreach ($pagos as $pago)
                                    <tbody>   
                                        <tr>
                                            <td class="alt" width="30" >{{$pago->id}}</td>
                                            <td class="alt" width="300" >{{$pago->pessoa->name}}</td>
                                            <td class="alt" width="80" style="text-align:center">{{$pago->tipo}}</td>
                                            <td class="alt" width="100" style="text-align:center">{{$pago->status_evento}}</td>
                                        </tr>  
                                    </tbody>                                  
                                @endforeach
                                <tr>
                                    <td colspan="3" style="widtd: 100%; text-align: right;"><strong>Total de Irmãs.:</strong></td>
                                    <td ><strong>{{$countPago}}</strong></td>
                                </tr>
                            </table>                                
                        @else
