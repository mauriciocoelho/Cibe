                        ...
                        @if ($gerals->count())
                            <table cellspacing="0" style="width: 100%; text-align: left; font-size: 10pt;">
                                ...
                                @foreach ($gerals as $geral)
                                    <tbody>   
                                        <tr>
                                            <td class="alt" width="30" >{{$geral->id}}</td>
                                            <td class="alt" width="300" >{{$geral->pessoa->name}}</td>
                                            <td class="alt" width="80" style="text-align:center">{{$geral->tipo}}</td>
                                            <td class="alt" width="100" style="text-align:center">{{$geral->status_evento}}</td>
                                        </tr>  
                                    </tbody>                                  
                                @endforeach
                                <tr>
                                    <td colspan="3" style="widtd: 100%; text-align: right;"><strong>Total de Irmãs.:</strong></td>
                                    <td ><strong>{{$countRegister}}</strong></td>
                                </tr>
                            </table>                                
                        @else
